...
echo =========== FUZZING
./pcre_asan_cov_fuzzer -drill=1 -max_len=$MAX_LEN $CORPUS  -artifact_prefix=CORPORA/ARTIFACTS/ -jobs=$J -workers=$J -max_total_time=$MAX_TOTAL_TIME -use_counters=1 > $L 2>&1
exit_code=$?
case $exit_code in
  0) prefix=pass
    ;;
